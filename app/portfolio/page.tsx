...
export default function Portfolio() {
  return <><SiteHeader /><main id="main-content" tabIndex={-1} className="cella-home pt-24">
    ...
    <CampaignResults />
    <section className="cella-section" aria-labelledby="case-studies-heading"><p className="cella-eyebrow">Behind the content</p><h2 id="case-studies-heading">EXPLORE THE STORIES.</h2><div className="mt-10 grid gap-x-10 sm:grid-cols-2">{projectLinks.map(project => <Link href={project.href} key={project.href} className="cella-text-link justify-between border-t border-[#34323c] py-6">{project.label}<ArrowUpRight size={18} aria-hidden="true" /></Link>)}</div></section>
