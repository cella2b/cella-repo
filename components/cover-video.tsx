  ...
  return (
    <>
      <div className="cella-cover-media" aria-hidden="true">
        ...
        <video ref={video} src={loadVideo ? "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/copy_609EA116-4303-4201-A986-82779EE24CCC-fSh9zYmlnaJhudebiyaAnN2AZzr5d0.mov" : undefined} muted loop playsInline preload="metadata" onCanPlay={(event) => { if (event.currentTarget.videoWidth > 0) setReady(true); else { event.currentTarget.pause(); setError(true); setPlaying(false) } }} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onError={() => { setError(true); setPlaying(false) }} className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-500 ${ready ? "opacity-100" : "opacity-0"}`} />
      </div>
